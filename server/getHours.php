...
if ($mes === 'SELECCIONAR') {
  if ($dia === 'SELECCIONAR') {
    $sql = "SELECT hour_entrance HORA, count(*) AFORO FROM {$table} WHERE date_entrance >= :inicio AND date_entrance <= :fin GROUP BY HORA";
    ...
    $sql = "SELECT hour_entrance HORA, count(date_entrance) AFORO FROM {$table} WHERE date_entrance IN ({$inClause}) GROUP BY HORA";
    $sentencia = $bd->prepare($sql);
    foreach ($dates as $idx => $d) {
  ...
  $fechaLike = "%{$fechaMes}%";
  $sql = "SELECT hour_entrance HORA, count(*) AFORO FROM {$table} WHERE date_entrance LIKE :fechaMes GROUP BY HORA";
  $sentencia = $bd->prepare($sql);
